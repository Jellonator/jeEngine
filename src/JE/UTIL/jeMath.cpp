...
namespace MATH{
	...
	bool compareDistanceFast(float x1, float y1, float x2, float y2, float distance){
		...
		return (squaredDistance(x1, y1, x2, y2) > distance*distance);
	}
	
	float Xangle(float angle, float distance, float x){
		return std::cos(RAD*angle)*distance+x;
	}
	
	float Yangle(float angle, float distance, float y){
		return std::sin(RAD*angle)*distance+y;
	}
